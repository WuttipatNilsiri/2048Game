...
public class ScoreManager {
		...
		try {
			...
			for (int row = 0; row < GameBoard.ROWS; row++) {
				for (int col = 0; col < GameBoard.COLS; col++) {
					if (row == GameBoard.ROWS - 1 && col == GameBoard.COLS - 1) writer.write("" + 0);
					else writer.write(0 + "-");
				}
			}
		...
		try {
			...
			for (int row = 0; row < GameBoard.ROWS; row++) {
				for (int col = 0; col < GameBoard.COLS; col++) {
					...
					if (row == GameBoard.ROWS - 1 && col == GameBoard.COLS - 1) writer.write("" + board[location]);
	...
	public void loadGame() {
		try {
			...
			for (int i = 0; i < board.length; i++) this.board[i] = Integer.parseInt(board[i]);
